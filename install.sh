...
# Written by: Dzero-M
# Facebook: https://www.facebook.com/varoudom.14/
# Github: https://github.com/DZER0-M
...
echo -e "$yellow          \/     \/            \/               \/       \/  \/  \/  "
echo -e "                                                                            "
echo -e "$orange                        Setup dzero-m v1.3.2                         "
echo -e "                                                                            "
echo -e "$orange                             By:DZER0-M                              "
...
# Shortcut for dzero-m
echo -e $yellow
echo -n [*] Configuración de acceso directo...= ;
sleep 3 & while [ "$(ps a | awk '{print $1}' | grep $!)" ] ; do for X in '-' '\' '|' '/'; do echo -en "\b$X"; sleep 0.1; done; done
echo ""
echo ""
echo -e "$green[!] ¿Do you want to be able to run dzero-m from anywhere in your terminal? (y/n)"
echo -e "$red"
...
lnk=$?
if [ "$lnk" ==  "0" ];then
run="cd $path && sudo python3 dzero-m.py"
touch /usr/local/bin/dzero-m
echo "#!/bin/bash" > /usr/local/bin/dzero-m
echo "$run" >> /usr/local/bin/dzero-m
chmod +x /usr/local/bin/dzero-m
cp images/dzero-m.desktop /usr/share/applications/dzero-m.desktop
cp images/dzero-m.png /usr/share/icons/dzero-m.png
sleep 2
echo -e $green
echo -e "╔──────────────────────────────────────────────────────────╗"
echo -e "|[✔] Installation complete. Type 'zero-m' to run the tool.|"
echo -e "┖──────────────────────────────────────────────────────────┙"
fi
;;
